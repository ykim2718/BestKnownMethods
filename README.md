...
Build, validate, fingerprint, and match Directed Acyclic Graphs that describe semiconductor wafer process flows from parquet data. Auto-discover BKM versions from a folder of wafer parquets, then route new wafers to a known BKM or flag them as `unknown`.

The package ships two surfaces:
- a Python class `BKM` for constructing and analyzing a single DAG, and
- a CLI / module `bkm-generator` (`bkm.generator`) for batch-processing a folder of wafers into a multi-BKM registry.
...
```bash
pip install bkm
pip install bkm[viz]   # adds graphviz for PNG / SVG rendering
```
...
## CLI quick start (`bkm-generator`)

Process every parquet in a folder and assign BKM versions:

```bash
# Discovery: build the BKM registry from training wafers
bkm-generator --data_folder ./training_data --update

# Inference: match new wafers against the existing registry
...
`python -m bkm.generator` works identically to the `bkm-generator` console script.

---

## Examples

A 4-script demo lives in [`examples/`](examples/). See [`examples/README.md`](examples/README.md) for the full walkthrough and I/O tables.

```bash
git clone https://github.com/ykim2718/BestKnownMethods.git
cd BestKnownMethods
pip install -e .[viz]

python examples/00_generate_sample_data.py     # synthetic wafer parquets
python examples/01_run_training.py             # CLI training (--update)
python examples/02_run_inference.py            # CLI inference (no --update)
python examples/03_python_api_demo.py          # Python API + branching BKM
```

---

# BKM class — single DAG

## 1. Definition

A **BKM** represents a semiconductor wafer process flow as a **Directed Acyclic Graph (DAG)**. Each node is a process step (called a **loaf**, e.g. a recipe name), and each directed edge represents the temporal execution order between steps.
...
- There are **no cycles** — you can never follow edges and return to a node you already visited.

These two properties make DAGs ideal for modeling process flows, because manufacturing steps always move forward in time and never loop back.
...
Every node in a DAG has two counts: **in-degree** (how many arrows point **into** it) and **out-degree** (how many arrows go **out of** it).
...
**Why `in` must have in-degree 0:** the `in` node is the very first step — nothing comes before it. If something pointed into `in`, that would mean there is a step before the start, which contradicts the definition of a starting point.
...
**Why `out` must have out-degree 0:** the `out` node is the very last step — nothing comes after it. If `out` pointed to something, that would mean the process continues after the end, which contradicts the definition of an ending point.
...
The core data structure for defining a BKM is `Dict[str, List[str]]`, mapping each node to its successor list.
...
Note: terminal nodes like `out` do not need to be listed as keys. The `_build()` method automatically creates `ProcessNode` entries for nodes that only appear as edge targets.
...
This reads all `.parquet` files in the folder, infers edges from each wafer using the predecessor detection algorithm (see §5), merges all edges into a single unified DAG, and calls `_build()`.
...
When reading a wafer's process steps from a parquet file, `read_wafer_edges()` infers the DAG edges from timestamps.

**Rule**: for each step X, the **predecessor** is the step whose `tkout` is the **latest** value that is still `<= X.tkin`.
...
Both `c` and `d` find `b` as their predecessor because `b.tkout (10:05) <= 10:10`. Neither `c` nor `d` can be predecessor of the other since they start at the same time.
...
The key insight: when step B is still running (`B.tkout > C.tkin`), step C cannot have B as its predecessor. Instead, C finds an earlier step as predecessor, which reveals that a **branch** occurred.
...
Two BKMs with identical graph structure produce identical fingerprints, regardless of version name or label configuration.
...
**Graphviz image** (requires `bkm[viz]` extra and system Graphviz binaries):
...
# bkm.generator — multi-BKM CLI

## 1. Purpose

`bkm.generator` (CLI: `bkm-generator`, also `python -m bkm.generator`) extends the `BKM` class from a **single-BKM tool** into a **multi-BKM batch processor**.

While the `BKM` class defines and manipulates a single BKM DAG, `bkm.generator` adds five capabilities the class does not have:

| Capability                    | `BKM` class                | `bkm.generator`                         |
|-------------------------------|-----------------------------|------------------------------------------|
...
## 2. How `bkm.generator` uses the `BKM` class

### 2.1 BKM as a reader (edge inference)

A `BKM` instance with no `process_data` is used purely as a **reader** to infer edges from a wafer DataFrame via `read_wafer_edges_from_df()`:
...
The reader does not hold any graph. It only uses the configured column labels to parse the DataFrame and return edges.

### 2.2 BKM as a registered version (DAG holder)

When a new BKM version is created, the edges are converted into `process_data` and a full BKM instance is constructed with a validated DAG:
...
This BKM holds a complete graph with fingerprint, validation, and all methods from the `BKM` class (matching, save/load, visualization).

### 2.3 BKM properties used by `bkm.generator`
...
## 3. Loaf construction from raw columns

The `BKM` class expects a single `recipe` (or `loaf`) column already present in the parquet. `bkm.generator` creates the loaf on-the-fly from multiple raw columns.
...
2. **Walks the columns in the order specified by `--bkm_columns`** (the user's order is preserved verbatim — no alphabetical sort), collecting sorted unique values from each column
3. Concatenates all values with `+` to form a single loaf string
4. Synthesizes `tkin_time` as `tkout_time - 60 minutes`

> **The order of `bkm_columns` is part of the BKM identity.** The same column set in a different order produces a different loaf string and therefore a different BKM (different fingerprint). Within a single column, however, values are always sorted — that's a canonical representation, not an order choice.
...
> `measured_value` is a real-life data column (sensor reading, etc.) that is **not part of the loaf composition** — only the columns listed in `--bkm_columns` (default: `equipment`, `chamber`, `chamber_step`, `sensor`) are used. Any extra columns in the parquet are simply ignored by `add_bkm_loaf()`.
...
If the user instead passes `--bkm_columns sensor,equipment,chamber_step,chamber`, the same data yields a **different loaf** (and a different BKM):
...
In the `BKM` class, the loaf is a single pre-existing column (e.g. `"recipe": "a"`). In real production data, there is no single "recipe" column. Instead, the process identity is determined by the **combination** of equipment, chamber, chamber step, and sensor. `bkm.generator` bridges this gap by composing the loaf from multiple columns before passing it to the BKM edge inference engine.

## 4. Multi-BKM registry

### 4.1 Data structures

The `BKM` class handles one BKM at a time. `bkm.generator` manages an **entire catalog** of BKM versions using two in-memory dictionaries:
...
To determine which BKM a wafer belongs to, the function extracts all loaf names from the wafer's edges (excluding virtual `in`/`out` nodes) and creates a `frozenset`:
...
This frozenset is looked up in `loaf_set_index`. Two wafers with the **exact same set of loaf names** are assigned to the same BKM version.
...
The `BKM` class saves a single BKM to a JSON file. `bkm.generator` writes **two sibling files** in the same `bkm_folder`:
...
The `loaf_set` field in `bkm_directory.json` stores the frozenset that was used for matching, enabling the registry to be reloaded and `loaf_set_index` to be reconstructed. `bkm_wafers.json` is informational output (not consumed by `load_bkm_directory()` — inference mode rebuilds matchings from the directory alone).
...
| `chamber`      | str      | Chamber (part of loaf)                   |
| `chamber_step` | str      | Chamber step (part of loaf)              |
...
Multi-version directory file saved to `bkm_folder/bkm_file`. Structure is described in §4.4.
